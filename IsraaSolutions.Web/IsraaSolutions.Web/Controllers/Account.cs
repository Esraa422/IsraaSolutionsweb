...
namespace IsraaApp.WebMVC.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public ActionResult Register()
        {
            ...
            model.UserRole = SystemRoles.Member;
            return View(model);
        }
        ...
        public async Task<ActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            var result = await AplicationUserService.ConfirmEmailAsync(userId, code);
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = await AplicationUserService.FindByNameAsync(model.Email);
                if (user == null || !(await AplicationUserService.IsEmailConfirmedAsync(user.Id)))
                {
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            var result = await AplicationUserService.ResetPasswordAsync(user.Id, model.Code, model.Password);
            if (result.Succeeded)
            {
                return RedirectToAction("ResetPasswordConfirmation", "Account");
            }
